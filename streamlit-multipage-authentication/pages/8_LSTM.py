...
if st.button("Fetch Data"):
    ...
    # Custom callback to capture final loss of each epoch
    class LossHistory(Callback):
        def on_epoch_end(self, epoch, logs=None):
            st.text(f'Epoch {epoch + 1}: loss = {logs["loss"]:.4f}')
    ...
    with redirect_stdout(train_output):
        history = model.fit(x_train, y_train, batch_size=1, epochs=3, callbacks=[loss_history])
